...
if (typeof woodPercentage == 'undefined') {
    woodPercentage = 40000 / 140000;
    stonePercentage = 50000 / 140000;
    ironPercentage = 50000 / 140000;
}
...
if (game_data.locale == "cs_CZ") {
    langShinko = [
        "Posílání surovin na šlechtu",
        "Vlož souřadnice cílové vesnice",
        "Uložit",
        "Tvůrce",
        "Hráč",
        "Vesnice",
    ...
    htmlCode = `
            <div id="sendResources" border=0>
                <table id="tableSend" width="100%">
                    <tbody id="appendHere">
                        <tr>
                            <td class="sophHeader" colspan=7 width=â€ś550â€ť style="text-align:center" >${langShinko[10]}</td>
                        </tr>
                        <tr>
...
function calculateResAmounts(wood, stone, iron, warehouse, merchants) {
    var merchantCarry = merchants * 1500;           // maximální teoretická kapacita
    var leaveBehindRes = Math.floor(warehouse / 100 * resLimit);

    // Dostupné suroviny po odečtení rezervy
    ...
    // === HLAVNÍ ÚPRAVA - TVRDÝ LIMIT 140k ===
    let maxTotal = 140000;

    // Respektujeme kapacitu obchodníků
    if (merchantCarry < maxTotal) {
        maxTotal = merchantCarry;
    }

    // Poměry (40 : 50 : 50)
    const woodRatio = 40000 / 140000;
    const stoneRatio = 50000 / 140000;
    const ironRatio = 50000 / 140000;

    let sendWood = Math.floor(maxTotal * woodRatio);
    let sendStone = Math.floor(maxTotal * stoneRatio);
    let sendIron = Math.floor(maxTotal * ironRatio);

    // Ořezání podle toho, co vesnice skutečně má
    if (sendWood > localWood) {
        let ratio = localWood / sendWood;
        sendWood = localWood;
        sendStone = Math.floor(sendStone * ratio);
        sendIron = Math.floor(sendIron * ratio);
    }
    if (sendStone > localStone) {
        let ratio = localStone / sendStone;
        sendStone = localStone;
        sendWood = Math.floor(sendWood * ratio);
        sendIron = Math.floor(sendIron * ratio);
    }
    if (sendIron > localIron) {
        let ratio = localIron / sendIron;
        sendIron = localIron;
        sendWood = Math.floor(sendWood * ratio);
        sendStone = Math.floor(sendStone * ratio);
    }

    // Finální pojistka, aby součet nikdy nepřekročil 140k (nebo kapacitu obchodníků)
    let total = sendWood + sendStone + sendIron;
    if (total > maxTotal) {
        // ubere přebytek z železa (nejméně důležitého pro mint)
        let excess = total - maxTotal;
        sendIron = Math.max(0, sendIron - excess);
    }

    return {
        wood: Math.floor(sendWood),
        stone: Math.floor(sendStone),
        iron: Math.floor(sendIron)
    };
}
